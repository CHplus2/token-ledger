...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-gradient-to-r from-[#ffffff] via-[#eef2ff] to-[#f5f3ff] text-slate-900 p-6 shadow-md border border-[#cbd5e1] relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-700 max-w-2xl leading-relaxed">
              Your tokenized asset books are <strong className="text-emerald-600 font-bold">{closeReadinessScore}% ready to close</strong>.
              {' '}Reconciliation across <span className="text-indigo-700 font-medium">issuer-custodied wallets (BlackRock, Khazanah, CIMB) and Coinbase/Kraken venues</span> stands at <strong className="text-emerald-700 font-bold">{reconciliationRatePercent}%</strong>.
              {openBreaksCount > 0 && (
                <> <strong className="text-amber-700">1 open exception</strong> on Backed NVDA (50-unit transfer variance) requires investigation.</>
              )}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              ...
              className="px-3.5 py-2 rounded-lg bg-[#e2e8f0] hover:bg-[#cbd5e1] border border-[#cbd5e1] text-xs font-semibold text-slate-900 transition-colors flex items-center gap-2 cursor-pointer"
            >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Five-Way Cross-Platform Reconciliation Health</h3>
            </div>
          ...
          <button
            onClick={() => setActiveModule('reconciliation')}
            className="text-xs text-purple-600 font-semibold hover:underline flex items-center gap-1 shrink-0"
          >
            ...
            >
              <div className="flex items-center justify-between mb-1.5">
                <span className="font-bold text-xs text-slate-900 flex items-center gap-1.5">
                  <span className="font-mono bg-[#e2e8f0] px-1.5 py-0.5 rounded text-[11px] text-purple-700 border border-[#cbd5e1]">
                    {rec.assetSymbol}
                  </span>
              ...
              {rec.varianceUnits !== 0 && (
                <div className="mt-2 pt-2 border-t border-amber-500/20 text-[10px] text-amber-700 font-medium flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#ffffff] rounded-xl border border-[#e2e8f0] p-5 shadow-xs flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Portfolio by Tokenized Instrument</h3>
              <p className="text-xs text-slate-600">Institutional fair value allocation</p>
            </div>
            <button
              onClick={() => setActiveModule('assets')}
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#e2e8f0] text-xs">
            {assetDistributionData.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between">
                ...
                <span className="text-slate-600 font-mono shrink-0">{item.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#ffffff] rounded-xl border border-[#e2e8f0] p-5 shadow-xs flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900">Position by Custody Venue</h3>
              <p className="text-xs text-slate-600">Issuer digital wallets, Solana Treasury, Coinbase & Kraken</p>
            </div>
            <button
          ...
          <div className="pt-2 border-t border-[#e2e8f0] text-xs text-slate-600 flex items-center justify-between">
            <span>Primary Active Ingestion: <strong className="text-slate-900">Solana Treasury (126 txs)</strong></span>
        ...
        <div className="bg-[#ffffff] rounded-xl border border-[#e2e8f0] p-5 shadow-xs flex flex-col">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-slate-600">Action items for Controller review</p>
            </div>
            <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-amber-500/10 text-amber-700 border border-amber-500/30">
          ...
          <div className="flex-1 space-y-2.5 overflow-y-auto max-h-60 pr-1">
            ...
            {openBreaksCount > 0 && (
              ...
              >
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="font-bold text-rose-700">50-Unit NVIDIA Equity Break</span>
                  <span className="font-semibold text-rose-700">$6,425 Variance</span>
                </div>
            ...
            {unclassifiedCount > 0 && (
              ...
              >
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="font-bold text-amber-700">Unclassified Inflow</span>
                  <span className="font-semibold text-amber-700">{unclassifiedCount} Items</span>
                </div>
            ...
            {pendingJournalsCount > 0 && (
              ...
              >
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="font-bold text-purple-700">Journals Awaiting Approval</span>
                  <span className="font-semibold text-purple-700">{pendingJournalsCount} Batches</span>
                </div>
          ...
          <div className="pt-3 border-t border-[#e2e8f0] mt-2">
            <button
              onClick={() => setActiveModule('reconciliation')}
              className="w-full py-1.5 rounded-lg bg-[#f8fafc] hover:bg-[#f1f5f9] border border-[#e2e8f0] text-slate-800 font-semibold text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
            >
      ...
      <div className="bg-[#ffffff] rounded-xl border border-[#e2e8f0] p-5 shadow-xs">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Recent Multi-Venue Subledger Activity</h3>
            <p className="text-xs text-slate-600">Institutional transactions across issuer digital wallets, Solana, Coinbase & Kraken</p>
          </div>
          <button
            onClick={() => setActiveModule('transactions')}
            ...
            <tbody className="divide-y divide-[#e2e8f0]">
              {transactions.slice(0, 5).map((tx) => (
                <tr
                  ...
                  className="hover:bg-[#f8fafc]/80 cursor-pointer transition-colors"
                >
                  ...
                  <td className="py-3 px-3 text-slate-600">
                    {tx.fromSource && tx.toSource ? (
                      ...
                      <span className="font-medium text-slate-800">{tx.sourceName}</span>
                    )}
                  </td>
