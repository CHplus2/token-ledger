...
export const TransactionDrawer: React.FC<TransactionDrawerProps> = ({ tx, onClose }) => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[540px] bg-[#ffffff] border-l border-[#e2e8f0] shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-200 overflow-hidden text-slate-900">
      {/* Drawer Header */}
      <div className="px-6 py-4 bg-[#ffffff] border-b border-[#e2e8f0] text-slate-900 flex items-center justify-between shrink-0">
        ...
        <button
          onClick={onClose}
          className="p-1.5 text-slate-600 hover:text-slate-900 rounded-lg hover:bg-[#f1f5f9] transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
        ...
        <div className="space-y-3">
          <div className="text-[11px] font-bold uppercase tracking-wider text-purple-600 flex items-center gap-1.5">
          ...
          <div className="bg-[#ffffff] rounded-xl p-4 border border-[#e2e8f0] space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-slate-800 font-mono">{tx.timestamp}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Legal Entity:</span>
              <span className="font-semibold text-slate-900">{tx.entityName}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Custody Location:</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Connected Source:</span>
              <span className="font-semibold text-slate-700">{tx.sourceName}</span>
            </div>
            {tx.walletName && (
              <div className="flex items-center justify-between">
                <span className="text-slate-600">Digital Wallet:</span>
                <span className="font-semibold text-slate-700">{tx.walletName}</span>
              </div>
            )}
            {tx.custodian && (
              <div className="flex items-center justify-between">
                <span className="text-slate-600">Custodian:</span>
                <span className="font-semibold text-purple-700">{tx.custodian}</span>
              </div>
            )}
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Tokenized Asset Movement:</span>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-[#e2e8f0]">
              <span className="text-slate-600">Recognized Fair Value (USD):</span>
            ...
            <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/30 space-y-2.5">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg bg-[#f8fafc] border border-purple-500/30 text-purple-700 font-medium text-xs hover:bg-[#f1f5f9] cursor-pointer"
                >
                  Explain Reason
                </button>
              </div>
          ...
          <div className="bg-[#ffffff] rounded-xl p-4 border border-[#e2e8f0] space-y-3">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-lg border border-[#cbd5e1] bg-[#f8fafc] font-medium text-slate-900 focus:ring-2 focus:ring-purple-500 focus:outline-none cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="p-2.5 bg-[#f8fafc] rounded-lg border border-[#e2e8f0]">
                <span className="block text-[10px] uppercase font-bold text-slate-600">Debit Account</span>
                <span className="font-bold text-slate-900">{tx.debitAccountCode}</span>
                <span className="block text-[11px] text-slate-600 truncate">{tx.debitAccountName}</span>
              </div>

              <div className="p-2.5 bg-[#f8fafc] rounded-lg border border-[#e2e8f0]">
                <span className="block text-[10px] uppercase font-bold text-slate-600">Credit Account</span>
                <span className="font-bold text-slate-900">{tx.creditAccountCode}</span>
                <span className="block text-[11px] text-slate-600 truncate">{tx.creditAccountName}</span>
              </div>
            </div>
            ...
            {tx.realizedGainLoss !== undefined && tx.realizedGainLoss !== null && (
              <div className="p-2.5 bg-emerald-500/10 rounded-lg border border-emerald-500/30 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-[11px] text-emerald-700">FIFO Acquisition Cost: ${(tx.costBasis ?? 0).toLocaleString()}</span>
                </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#ffffff] rounded-xl p-4 border border-[#e2e8f0] space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Reconciliation Match:</span>
              <StatusBadge status={tx.reconciliationStatus} size="sm" />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Matching Method:</span>
              <span className="font-medium text-slate-800 font-mono">
                {tx.reconciliationMatchMethod || 'EXACT_HASH'}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-600">Pricing Oracle / Valuation Feed:</span>
              <span className="font-medium text-slate-800">{tx.priceSource}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#ffffff] rounded-xl p-4 border border-[#e2e8f0] space-y-2.5">
            <div>
              <span className="block text-slate-600 mb-1">DLT Transaction Reference:</span>
              <div className="flex items-center justify-between p-2 bg-[#f8fafc] rounded-lg border border-[#e2e8f0] font-mono text-[11px] text-slate-800">
                <span className="truncate max-w-[360px]">{tx.txHash}</span>
                <button
                  onClick={handleCopyHash}
                  className="p-1 text-slate-600 hover:text-slate-900 ml-2 cursor-pointer"
            ...
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-slate-600">Infrastructure / Platform:</span>
              <span className="font-semibold text-slate-900">{tx.network}</span>
            </div>
            ...
            <div className="pt-1">
              <button
                ...
                className="text-[11px] font-semibold text-purple-600 hover:underline flex items-center gap-1 cursor-pointer"
              >
              ...
              {showTechnicalDetails && (
                <pre className="mt-2 p-3 bg-black/90 text-purple-700 rounded-lg text-[10px] font-mono overflow-x-auto max-h-40 border border-[#e2e8f0]">
                  {JSON.stringify(
                    {
                      txId: tx.id,
      ...
      <div className="p-4 bg-[#ffffff] border-t border-[#e2e8f0] flex items-center justify-between shrink-0">
        <div className="text-[11px] text-slate-600">
          Status: <strong className="text-slate-900">{tx.accountingStatus}</strong>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="px-3 py-2 rounded-lg border border-[#cbd5e1] hover:bg-[#f1f5f9] text-slate-700 font-medium text-xs cursor-pointer"
          >
            Close
          </button>
        </div>
